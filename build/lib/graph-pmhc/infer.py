...
import gpmhc.GNN_paper.data as data_utils
import gpmhc.GNN_paper.learner as learner_utils

#Only one GPU inference is currently suppported, cpu inference not supported
def infer(input_model='/gstore/data/gai/pcv/thriftw/trained_models/mhc2_gnn/v0_0_1_baseline_33/4/', sub_model='model_final',csv='/gstore/data/gai/pcv/thriftw/mhc2_small_df.csv', arch='GNN_paper.baseline_model'):
    #for naming files
    ...
    #load weights
    learner = learner.load(file=input_model + sub_model)
    ...
    #Generate dataloader
    test_dl = learner.arch.get_dataloaders_test(df, use_gpu)
    
    #Do inference
    with learner.parallel_ctx():  y_pred, y_true = y_pred, y_true = learner.get_preds(dl=test_dl[0], act=PassThroughLayer, reorder=False)
